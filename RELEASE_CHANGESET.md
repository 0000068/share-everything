...
- CI tests the exact Node 22.13.0 lower boundary and Node 24, then runs a strict Linux Chrome structure/behavior contract. Chrome readiness has a bounded 30-second shared-runner cold-start window. The real-browser 320px desktop checks verify desktop device metrics, UA shape, and actual geometry without depending on headless Linux exposing a physical fine pointer or assuming desktop hardware has zero touch points; deterministic SiteUtils smoke separately locks fine-pointer classification, including legacy `ontouchstart`, and real mobile scenarios cover the coarse-pointer branch. Local release verification retains Windows pixel baselines for all seven shared scenarios and fails closed if any is absent. Workflow permissions are read-only, checkout credentials are not persisted, and reviewed action releases are full-SHA pinned.
