...
| `A-2` | **SSR 模板从正则替换迁移到 jsdom / parse5**。`api/post.js` 仍用 ~10 个正则模式做模板替换。已落地：HEAD meta block 走 `SSR_HEAD_META_START/END` 标记；postContent / postEmpty / data-empty-link 锚点在 smoke check 强制存在；属性顺序无关的 empty-state 替换 regex。**触发迁移**：模板再发生一次"regex 兼容性陷阱"或者需要更复杂的局部 DOM 操作。 |
