...
> 更新时间：2026-09-12（v8.8 异常恢复、并发与内容完整性修复）

---

## 一、当前待修任务

> 9 月 12 日审计的 A01–A12 已逐项修复，并补充行为回归；同时收紧静态发布范围并修订回滚说明。修复内容、验证结果及线上验收记录位置统一记录在 `RELEASE_CHANGESET.md`。
...
| `B-4` | **历史 deployment 的 Git source 失效**。旧记录曾指向已不可用的仓库，不能假设 v7.x 的 Redeploy 可用。需要恢复旧行为时，优先使用仍可用且已验证的 deployment；否则通过回退提交恢复所需代码，保留 `main` 历史，重新同步生成资源、运行完整门禁，再创建部署。不强制推送旧提交覆盖主分支。 |

---

## 三、历史完成记录

### v8.8 异常恢复与内容完整性（2026-09-12）

- A01：Sharp 取消不再触发独立 stream error；请求等待可取消，原生转换有处理预算，并发槽直到真实转换结算后才释放。
- A02/A03：独立启动守卫覆盖静态依赖、入口和动态页面模块失败；连续导航不复用已取消请求。
- A04/A05：快速重新收藏保留完整元数据，精简摘要明确待补全，收藏代次 8 修复旧记录；分类冷查询按分类合并，保留订阅者取消、失败冷却及容量上限。
- A06/A07/A08/A09：TeX 单 token 参数正确解析；移动长行内公式可滚动；锚点在正文和进入动画就绪后定位；修改时间贯穿文章摘要、SSR/客户端结构化数据及 sitemap。
- A10/A11/A12：非法 URL 返回 400 且开发进程继续服务；品牌检查比较解析后的文本及属性；修饰键和非主键点击保留浏览器行为。
- 静态产物采用白名单，服务器代码和工具不再发布为静态资源；保留函数构建依赖。最终验证见 `RELEASE_CHANGESET.md`。

### v8.7 文章访问故障与全量审查修复（2026-09-09）

- 修复 Vercel 重写向规范文章 URL 注入 `id` 后触发的自身重定向循环；SPA 增加直接 SSR 端点恢复路径，保留公开地址，重定向响应改为浏览器必须重新验证。
- 文章 HTML 的请求预算从 15 秒提高到 35 秒；首屏 bootstrap 保留服务端诊断信息和限流等待时间。
- 修复列表轻量模块加载时收藏封面签名丢失；标记不完整会话摘要，阻止覆盖完整收藏，元数据代次升到 7 以修复旧记录。
- 表格行只渲染一次；回收站和归档页面不通过公开校验。
- 修复 SPA 分类/收藏页 SEO 被模板覆盖和翻页 canonical 未更新；分类标识保持 128 字符；首页保留生成的精选分类配置。
- 升级 Sharp、PostCSS 和受影响传递依赖，将 `parse5` 移到生产依赖；补充跨真实模块、刷新存储、重写请求形态和慢响应的集成回归。

### v8.6 性能、取消链路与发布门禁加固（2026-07-18）
