...
- Hardened the Linux Chrome contract after the first synchronized v8.6 run exposed two runner-specific assumptions: browser readiness now has a bounded 30-second cold-start window, and the real-browser 320px desktop contract verifies desktop device metrics, UA shape, and geometry without pretending a headless runner has a physical fine pointer or that desktop hardware cannot have touch points. Deterministic SiteUtils smoke retains exact fine-pointer plus legacy `ontouchstart` classification coverage, while real mobile scenarios retain the coarse-pointer compatibility branch.
